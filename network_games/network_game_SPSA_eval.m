...
function loss = network_game_loss(theta,theta_norm,coef_map,base_coef,loss_metric,carriers,market_data_mat,fixed_carrier,fixed_market_carriers,fixed_markets,num_carriers,segment_competitors,Market_freqs,empirical_freqs)
           ...
           elseif  (mkt_size==3)  
               ...
               for i = 1:3
                   if (freq_ind == i)
                       ...
                       transcoef(i+4) = -(Mnew/Mold)*base(7);
                   end
               end
               %interaction fs
               if (freq_ind == 1)
    ...
    for mk=1:numel(segment_competitors)
        ...
        for row=row_ind:row_ind+segment_competitors(mk) - 1
            freq_results_mat(row,1)=market_data(rank);            
        end
