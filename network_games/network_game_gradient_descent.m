...
for k=0:n-1   
    ...
    eps=c/(k+1)^gamma;
    tic
    gradient = zeros(numel(theta),1);
    y= network_game_loss_quadratic(theta,theta_norm,coef_map,base_coef,loss_metric,carriers,market_data_mat,fixed_carrier,fixed_market_carriers,fixed_markets,num_carriers,segment_competitors,Market_freqs,empirical_freqs,0,MAPE_incl,outfile_fn);
    for i=1:numel(theta)
        eps_vector = zeros(numel(theta),1);
        eps_vector(i)=eps;
        y_eps = network_game_loss_quadratic(theta + eps_vector,theta_norm,coef_map,base_coef,loss_metric,carriers,market_data_mat,fixed_carrier,fixed_market_carriers,fixed_markets,num_carriers,segment_competitors,Market_freqs,empirical_freqs,0,MAPE_incl,outfile_fn);
       
        gradient(i) = (y_eps-y)/eps;
    end       
    ys(1,k+1)=y_eps;
    ys(2,k+1)=y;  
