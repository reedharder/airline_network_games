...
def create_route_demands_quarter2(year =2014, quarter=1, filter_null_fares=False, filter_fare_bounds = [None, None], base_coup_fn= 'DB1B_COUPON_{year}_Q{quarter}.csv', base_mkt_fn = 'DB1B_MARKET_{year}_Q{quarter}.csv',data_dir = 'C:/users/d29905p/Documents/airline_competition_paper/code/network_games/bts_data/'):
    ...
    merge_df  =  merge_df['ORIGIN','CONNECTION','DESTINATION','FIRST_OPERATING_CARRIER','SECOND_OPERATING_CARRIER','YEAR','QUARTER','NUM_FLIGHTS','PASSENGERS'].groupby(['MARKET_ID']).aggregate({'YEAR': lambda x: int(x['YEAR'].iloc[0]),'QUARTER': lambda x: int(x['QUARTER'].iloc[0]),'NUM_FLIGHTS': lambda x: int(x['NUM_FLIGHTS'].iloc[0]),'PASSENGERS': lambda x: np.sum(x['PASSENGERS'])*10}).reset_index()  
        
    
    merge_df  =  merge_df['ORIGIN','CONNECTION','DESTINATION','FIRST_OPERATING_CARRIER','SECOND_OPERATING_CARRIER','YEAR','QUARTER','NUM_FLIGHTS','PASSENGERS'].groupby(['ORIGIN','CONNECTION','DESTINATION','FIRST_OPERATING_CARRIER','SECOND_OPERATING_CARRIER']).aggregate({'YEAR': lambda x: int(x['YEAR'].iloc[0]),'QUARTER': lambda x: int(x['QUARTER'].iloc[0]),'NUM_FLIGHTS': lambda x: int(x['NUM_FLIGHTS'].iloc[0]),'PASSENGERS': lambda x: np.sum(x['PASSENGERS'])*10}).reset_index()  
